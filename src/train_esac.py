...
parser.add_argument('--hypotheses', '-hyps', type=int, default=256, help='number of hypotheses, i.e. number of RANSAC iterations')
...
parser.add_argument('--inlieralpha', '-ia', 
    type=float, 
    default=100, 
...
parser.add_argument('--weightrot', '-wr', 
    type=float, 
    default=1.0,  
    help='weight of rotation error (in degree)'
)

parser.add_argument('--weighttrans', '-wt', 
    type=float, 
    default=100.0,  
    help='weight of translation error (in meters)'
)

parser.add_argument('--losscut', '-lc', 
    type=float, 
    default=100.0, 
...
parser.add_argument('--refined', '-ref', 
    action='store_true', 
...
if opt.clusters < 0:
    ...
    ensemble = ExpertEnsemble(trainset.num_experts, lr=opt.learningrate, gating_capacity=2)


trainset_loader = torch.utils.data.DataLoader(trainset, shuffle=True, num_workers=6)
ensemble.load_experts(opt.append, opt.refined)

if opt.expertselection:
    model_file = 'es_%s.net' % (opt.append)
    train_log = open('log_es_%s.txt' % (opt.append), 'w', 1)
else:
    model_file = 'esac_%s.net' % (opt.append)
    train_log = open('log_esac_%s.txt' % (opt.append), 'w', 1)
...
train_log.close()
print('Final stage training finished.')
